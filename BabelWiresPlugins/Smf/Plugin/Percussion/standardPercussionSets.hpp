...
namespace smf {
    class StandardPercussionSets {
      public:
        ...
        const seqwires::PercussionSet* getDefaultPercussionSet(GMSpecType::Value gmSpec, int channelNumber);
        ...
        const seqwires::PercussionSet* getPercussionSetFromChannelSetupInfo(GMSpecType::Value gmSpec, ChannelSetupInfo channelSetupInfo);

        /// Find the percussion set which contains the most instruments from the given set, and is suitable for use in
        /// the given channel.
        const seqwires::PercussionSet* getBestPercussionSet(GMSpecType::Value gmSpec, int channelNumber,
        ...
        std::optional<ChannelSetupInfo> getChannelSetupInfoFromPercussionSet(const seqwires::PercussionSet* percussionSet, int channelNumber);
      ...
      private:
        ...
        const seqwires::PercussionSet* getBestPercussionSetInRange(int startIndex, int endIndex,
                                                  const std::unordered_set<babelwires::Identifier>& instrumentsInUse,
                                                  std::unordered_set<babelwires::Identifier>& excludedInstrumentsOut);

        /// Find the percussion set in the array of known percussion sets. Asserts if not found.
        KnownPercussionSets getKnownPercussionSetFromPercussionSet(const seqwires::PercussionSet* percussionSet);
      ...
      private:
        std::array<const seqwires::PercussionSet*, NUM_KNOWN_PERCUSSION_SETS> m_knownSets;
